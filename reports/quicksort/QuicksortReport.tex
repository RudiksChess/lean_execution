...
\newcommand{\Sorted}{\operatorname{\mathsf{Sorted}}}
...
final theorem rests only on Lean's three standard classical axioms.
\end{abstract}
...
\begin{definition}[Sorted]\label{def:sorted}
...
After \texttt{termination\_by l => l.length}, Lean asks us to prove that each recursive argument
is smaller in the well-founded order. The tactic \texttt{simp\_wf} reduces each goal to
\[
  \len{\filt(q)(r)}\ \leq\ \len{r},
\]
which is closed by \texttt{length\_filter\_le} (Lemma~\ref{lem:filter-le}); the trailing
\texttt{le\_of\_eq (by simp)} absorbs a bookkeeping rewrite of the length (the internal
\texttt{attach} used to remember that filtered elements come from $r$ satisfies
$\len{r.\mathtt{attach}}=\len{r}$).
...
permutation, using Lean's notation \texttt{\~{}} for \texttt{List.Perm}) and \texttt{Sorted
...
\subsection{Lean}
The proof follows the mathematics line for line. The two \texttt{have}s are the induction
hypotheses (recursive calls on the shorter halves); \texttt{ih1.append (ih2.cons p)} is the
congruence step; \texttt{perm\_middle} pulls the pivot to the front; and
\texttt{filter\_append\_perm} is Lemma~\ref{lem:filter-perm}.
...
the small half is $\leq p$ and every element of the large half is $>p$ (hence $\geq p$). Combined
...
\subsection{Lean}
The Lean proof names the same pieces: \texttt{ih1}, \texttt{ih2} are $\Sorted(A)$, $\Sorted(B)$;
\texttt{hsmall}, \texttt{hlarge} are the two halves of $(\ast)$, each obtained by transporting
membership back through \texttt{mem\_quicksort} (Corollary~\ref{cor:mem}) and unfolding the filter
...
\noindent The three reported axioms---\texttt{propext} (propositional extensionality),
\texttt{Classical.choice} (choice), and \texttt{Quot.sound} (soundness of quotients)---are the
standard classical foundations underlying all of Mathlib; they are not specific to this
development. Crucially, \texttt{sorryAx} does \emph{not} appear: there are no admitted gaps, and
...
\paragraph{Protocol.} Aristotle was given only the definition of \texttt{Sorted}, the recursive
...
Every artifact below compiles under the same Lean~v4.29.0 toolchain as the rest of this development and
...
permutation. Decisively, the axiom audit returns the identical verdict: \texttt{quicksort\_correct}
depends only on \texttt{[propext, Classical.choice, Quot.sound]}, with no \texttt{sorryAx}. An
independent agent, handed only the algorithm, converges on the same theorem carrying the same trust
guarantee.
