...
the input. The helper \texttt{filter\_length\_lt\_cons} combines the two bounds
\[
...
\subsection{Lean}
The hypotheses \texttt{ihSmall} and \texttt{ihLarge} are recursive calls on the shorter
partitions. The named steps \texttt{hRight}, \texttt{hAppend}, \texttt{hPivot}, and
\texttt{hPartition} expose the congruence and partition arguments; the final
...
\subsection{Lean}
The Lean proof names the same pieces: \texttt{ihSmall}, \texttt{ihLarge} are $\Sorted(A)$, $\Sorted(B)$;
\texttt{hSmallBound}, \texttt{hLargeBound} are the two halves of $(\ast)$, each obtained by transporting
...
\texttt{Quot.sound} (soundness of quotients). The explicit termination proof no longer
